...
function Agendas() {
    ...
    const editCita = (Nombremodelo) => {
        history('/agenda/' + Nombremodelo);
    }

    const search = () => {
        let result = searchAgendas();
        setAgendas(result);
    }

    const remove = (Nombremodelo) => {
        removeAgenda(Nombremodelo);
        search();
    }
        ...
        {/* <Table striped bordered hover className='bg-light text-center container'>
            ...
            <div className='container'>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    {agendas.map(agenda => <tbody>
                        <tr class="table-light">
                            <th scope="row">{agenda.Nombremodelo}</th>
                            <td>{agenda.Telefono}</td>
                            <td>{agenda.Fecha}</td>
                            <td>{agenda.Hora}</td>
                            <td>{agenda.Servicio}</td>
                            <td>
                                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => remove(agenda.Nombremodelo)} color='red' type='button' width="16" height="16" fill="currentColor" class="bi bi-calendar-x me-4" viewBox="0 0 16 16">
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" onClick={() => editCita(agenda.Nombremodelo)} color='blue' type='button' width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
